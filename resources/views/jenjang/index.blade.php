        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{ route('jenjang.create') }}" class="btn btn-outline-primary btn-sm ml-5">Tambah Jenjang</a>
                </div>
                @if(session()->has('msg'))
                        ...
                        @foreach ($jenjang as $index => $item)
                            <tr>
                                ...
                                <td class="text-center">
                                    <a class="icon" href="{{ route('jenjang.edit', $item->id) }}" title="edit item">
                                        <i class="fe fe-edit"></i>
                                    </a>
                                    <a class="icon btn-delete" href="#!" data-id="{{ $item->id }}" title="delete item">
                                        <i class="fe fe-trash"></i>
                                    </a>
                                    <form action="{{ route('jenjang.destroy', $item->id) }}" method="POST" id="form-{{ $item->id }}">
                                        @csrf 
                                    </form>
                                </td>
                            </tr>
        ...
        $(document).ready(function () {
            ...
            $(document).on('click','.btn-delete', function(){
                ...
                swal({
                    ...
                    text: 'jenjang yang dihapus tidak dapat dikembalikan',
                    dangerMode: true,
                    buttons: {
                        cancel: true,
                        confirm: true,
